...
async def search(body: SearchRequest, request: Request):
    ...
    try:
        ...
        documents = await db.search(
            vector, int(os.getenv("RETRIEVAL_LIMIT", "10")), client
        )
        documents = await jev.score_documents(documents, questions, client)
        answer = await llm.generate_answer(query, questions, documents, client)
        return {
            "query": query,
            "questions": questions,
            "documents": documents,
